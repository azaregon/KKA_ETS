...
def load_geojson(fname:str):
    with open(fname,"r") as geojson_file:
        ret_val =  json.load(geojson_file)
...
def generate_map_graph():
    ...
    geojson = load_geojson("feed-data/polygon-bounds-geojson.json")
    ...
    with open('map-data.pkl', 'wb') as pickle_file:
        G = pickle.dump(G, pickle_file)
